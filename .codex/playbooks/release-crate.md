...
<sanitized-gh-environment> "<canonical-gh-executable>" api repos/bfeintuch123/crate-app/pulls/<release-pr-number> --jq '{number,state,merged,base:{ref:.base.ref},head:{ref:.head.ref,sha:.head.sha},merge_commit_sha}'
...
Require the bounded release-PR object to report `state: closed`, `merged: true`, `base.ref: v2.4.x`, `head.ref: <release-prep-branch>`, `head.sha: <release-pr-head-sha>`, and `merge_commit_sha: <release-merge-sha>`. Require exactly one successful check-run object and exactly one successful workflow-run object at both that version-only PR head SHA and that protected-branch release merge SHA. For each SHA, their check-suite IDs must be equal, the workflow path must be `.github/workflows/security-gate.yml`, and the app slug must be `github-actions`. The PR-head workflow event must be `pull_request` and its head branch must be `<release-prep-branch>`; the merge workflow event must be `push` and its branch must be `v2.4.x`.
...
10. After one combined build, signing, app-notarization, app-stapling, and app-staple-validation approval, build under the exact sanitized Node environment with `"<canonical-node-executable>" scripts/run-electron-builder-release.js --mac --arm64 --config.npmRebuild=false --config.publish.provider=github --config.publish.owner=bfeintuch123 --config.publish.repo=crate-app --publish never` from that exact commit. The explicit repository configuration works in detached release worktrees and generates local update metadata, while the exact `never` policy keeps Electron Builder's upload scheduling disabled. The launcher reauthenticates the canonical Node bytes, binds them to the running process, and forces Electron Builder's built-in in-process filesystem traversal collector over the already reconstructed production dependency tree. It also fails closed if Electron Builder requests an npm subprocess, so build-time dependency collection cannot run install, rebuild, lifecycle, version, publish, or other npm commands. The fresh lockfile-integrity-reconstructed Electron Builder CLI runs under canonical Node with child lookup limited to standard macOS system paths. This prevents Node/npm/script-shell injection and package-local executable lookup without an additional dependency-rebuild path. The signed-app verifier remains the hard byte-authentication gate for the resulting application; this build-tool reconstruction is not a substitute for that proof. Its configured `afterSign` hook at `scripts/notarize.js` must suppress private `electron-notarize*` debug output, notarize, staple, and validate the app before Electron Builder creates the DMG and ZIP. Electron Builder must sign the completed DMG with the configured Developer ID Application identity before generating its initial metadata. A build-only approval is insufficient.
11. With separate approval, revalidate the reviewed Keychain profile with `/usr/bin/xcrun notarytool history --keychain-profile "crate-release-notarytool"`. Before submission, require `/usr/bin/codesign --verify --strict --verbose=2 <dmg>` to validate the completed Developer ID-signed DMG; do not require pre-notarization Gatekeeper acceptance because `spctl` reports a valid signed artifact as `Unnotarized Developer ID` until Apple issues its ticket. Notarize the verified DMG with `/usr/bin/xcrun notarytool submit <dmg> --keychain-profile "crate-release-notarytool" --wait`, attach the accepted ticket with `/usr/bin/xcrun stapler staple <dmg>`, verify it with `/usr/bin/xcrun stapler validate <dmg>`, and then require `/usr/sbin/spctl -a -t open --context context:primary-signature -v <dmg>` to accept the final stapled DMG. Because stapling changes the DMG bytes, immediately run `scripts/finalize-mac-release-metadata.js` under the authenticated canonical Node environment to regenerate only the final DMG blockmap and `latest-mac.yml` from the final DMG and unchanged ZIP bytes. Require `dist/.crate-release-metadata-incomplete` to be absent afterward; its presence is a hard stop that requires a clean rebuild. Do not submit the app a second time after the successful `afterSign` app notarization, and do not publish pre-staple update metadata.
...
15. Validate final DMG/ZIP names, sizes, SHA-256 hashes, `latest-mac.yml` entries and checksums, and every required blockmap against those container-verified artifacts. Require the finalizer to leave the ZIP and ZIP blockmap byte-identical to their post-build values.
