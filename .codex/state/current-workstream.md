...
- Active app branch: `codex/final-security-audit-closeout`
- Current security work: close durable evidence after setting `v2.4.x` as the protected default branch with the existing source-security check and a zero-approval ordinary-PR requirement, then run a contained Mac mini QA build and installed-app smoke at the exact audited source. Before a `v3.0.0-beta.2` version PR or distributable tester release, add the independently controlled release approval and stronger ruleset/tag protections required by the release gate.
...
- Next action: complete this documentation/governance closeout, then run a contained Mac mini QA build and targeted installed-app smoke covering launch/recovery, Keychain and Figma migration, Quick Package, one standard package, diagnostics/privacy, and unchanged core behavior. Before preparing `v3.0.0-beta.2` for Olivia, establish the independently controlled release approval and full release-governance rules required before version mutation. Olivia's real-workflow testing and resulting triage come before updater, billing, website, legal, support, observability, or other public-release work.
