...
- Current phase: user-data security hardening. Phases 1 through 6A are merged. Phase 6B CI enforcement and signed-artifact proof is in its final failure loop on PR #140 against `v2.4.x`; live CI, GitHub review, and independent Autoreview exposed job-level expression, legacy-gitlink checkout, npm configuration-isolation, release-playbook npm isolation, workflow ordering, verifier-toolchain trust, incomplete Canvas-download, temporary-state authorization, missing clean-install integration, premature proof-output, and Canvas rollback defects. Every identified source finding is locally corrected with failure-first coverage, and the final narrow independent rereview reports no actionable findings. Fresh amended PR CI and merge-readiness review remain mandatory before separate merge approval.
- Active app branch: `codex/security-release-proof-phase6b`
- Current security work: make Phase 6A artifact protections repeatable and fail closed through a source-only CI gate, strict install-script policy, approved Canvas prebuild path, privacy-safe notarization, and an exact-revision signed-app proof without changing Crate's user workflow.
...
- Phase 6B scope: no app runtime, renderer, package, watcher, parser, provenance, diagnostics, quota, Figma, dependency, or lockfile change. Bryant approved one commit, branch push, and PR #140; no merge, build, install, signing, notarization, release, deployment, updater, or tester mutation has occurred.
...
- Next action: amend and force-push the single approved PR #140 commit with the reviewed rollback closure, require fresh Node 22 source-security CI, resolve the addressed inline review thread with evidence, and complete final merge readiness. Stop for separate merge approval before any merge, build, signing, installed-app mutation, release, deploy, updater, dependency mutation, or Olivia restart.
