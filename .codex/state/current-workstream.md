...
- Current security work: close durable evidence after setting `v2.4.x` as the protected default branch with the existing source-security check and a zero-approval PR requirement, then prepare a version-only `v3.0.0-beta.2` candidate for a separately approved signed build and installed-app smoke.
...
- Next action: complete this documentation/governance closeout, prepare a version-only `v3.0.0-beta.2` PR, and stop for the separately approved build/sign/notarize gate. Run a targeted installed-app smoke covering launch/recovery, Keychain and Figma migration, Quick Package, one standard package, diagnostics/privacy, and unchanged core behavior before Olivia resumes. Olivia's real-workflow testing and resulting triage come before updater, billing, website, legal, support, observability, or other public-release work.
