...
- Current phase: user-data security hardening. Phases 1 through 6B are merged. The separately deferred moderate `uuid@9.0.1` advisory cleanup is active on a clean exact-base worktree before the final integrated six-phase audit. The candidate replaces the two existing `uuid.v4()` calls with Node `crypto.randomUUID()`, removes only the direct dependency and lock record, and preserves app, package, Figma, provenance, quota, and renderer behavior.
- Active app branch: `codex/security-uuid-cleanup`
- Current security work: remove the direct `uuid` production dependency with the smallest runtime-equivalent change, prove package/lock isolation and deterministic test behavior, and stop before staging for Bryant review.
...
- Next action: the frozen UUID cleanup candidate is ready for Bryant's separate approval to stage, commit, push, and open a PR against `v2.4.x`. Stop before those actions and before build, install, release, deploy, updater work, or Olivia restart.
