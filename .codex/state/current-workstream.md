...
- Current phase: user-data security hardening. Phases 1 through 6B and the deferred `uuid` advisory cleanup are merged. A separate Crate ops repair is active before the final integrated six-phase audit because the persistent task bridge still referenced the removed Codex app path.
- Active app branch: `codex/ops-thread-bridge-discovery`
- Current ops work: make the local persistent task bridge discover the current ChatGPT/Codex executable safely, prove the fallback order and failure behavior, and validate the live bridge before returning to the final integrated security audit.
...
- UUID cleanup merge: PR #141 merged into `v2.4.x` as `29a651619fdd6c47396fc97dd7e150e5bdddfd95` after fresh CI and merge readiness passed. Both full and production-only audits report zero vulnerabilities.
...
- Next action: publish the no-finding executable-discovery repair through a PR against `v2.4.x`, require fresh CI and merge readiness, and merge only if clean. Then return to the final integrated six-phase audit; no build, install, release, deploy, updater work, or Olivia restart.
