...
Fresh source task from canonical Beta 2.24 `72a06a0`, after Bryant reported the installed Existing Assets dialog worked. Adds one background native notice for a newly actionable decision and an independent default-on Settings toggle that respects global notification disable. Clicks are account/project/activation guarded and never decide or change exclusions. Watching/polling continues; packaging waits for Include/Skip. Taskflow: `.codex/taskflows/2026-09-20-existing-assets-native-notification.md`. Initial exact-head CI passed; lifecycle review found a delayed-notification tab-navigation race, addressed in normal correction cycle 1 with a failure-first regression. Correction cycle 2 additionally fixes a reproduced two-notice response-order race so the latest admitted click wins; fresh exact-head CI and both reviews are required. Final correction cycle 3 extends the same intent guard to nested Settings section click/keyboard navigation, with failure-first coverage; remaining findings require escalation after fresh gates. Fresh exact-head checks/reviews/draft identity are recorded externally; no live app, OS permission, owner data, build, ready/merge or delivery action.

## 2026-09-20 — Figma existing-assets decision correction

Fresh correction from canonical `v2.4.x@fca609d` under Bryant's September 20 authorization. Complete synthetic first import reproduces bulk `asset_baseline_decision_unavailable`; scoped lifecycle and explicit include/skip UI correction is tracked in `.codex/taskflows/2026-09-20-figma-existing-assets-decision.md`. Final-head checks, two Luna/high review receipts, draft PR, and remaining visual gate live externally in the PR/native persistent September 20 receipt. The initial exact-head CI passed; UI review identified a partial/all-excluded dialog mismatch, addressed in normal follow-up cycle 1 with a failure-first regression and 352/352 local Figma/renderer checks. Cycle 1 CI and 938/938 local serial tests passed; cycle 2 preserves the attempted choice on save-error keyboard retry, with failure-first coverage and 352/352 focused checks. Fresh final-head CI/reviews remain externally recorded; historical acceptance is not inferred from current correction tests. No owner data, live app/auth, build, version, release, ready, or merge actions.
...
### 2026-07-15 Phase 5A Local Data Lifecycle

- PR #136 merged the contained Phase 4 installed-app follow-up into `v2.4.x` as `0c99cb2b897fbb3c1997a0590609ef33a24985d8`.
- Bryant approved splitting Phase 5 into two independently reviewable slices and beginning Phase 5A with mandatory autoreview before commit or PR work.
- Created clean branch `codex/security-local-data-lifecycle-phase5a` from the merged Phase 4 follow-up.
- Kept electron-store's existing atomic write path and added fail-closed owner-only verification for the real config file and Crate user-data directory before project state is opened. A fixed native error remains visible when storage cannot be secured, avoiding a silent zero-window failure.
- Added bounded cleanup for only `~/.crate/figma-assets/<project-id>` and `~/.crate/presentation-assets/<project-id>` after project deletion, delete-all, and validated startup orphan detection.
- Cleanup preserves every valid current project and unrecognized directory, rejects symlinked roots and categories, verifies realpath containment and owner-only modes, atomically quarantines a validated cache directory, retries transient rename or removal failures, and retains a safe next-launch retry path without exposing local paths.
- A pre-review race check proved an in-flight Figma download could recreate a cache after project deletion; a failure-first test now verifies the settled scan removes that late cache without restoring the deleted project.
- Failure-first local-data coverage failed 4/4 before implementation. Five adversarial review rounds then added native startup-error coverage, strict store-path verification, corrupt-store containment, transient cleanup retry coverage, Crate-owned orphan recognition, deterministic queue-drain sentinels, hard-link and final/intermediate-symlink containment, reactivation-during-quarantine restoration, unsafe-entry batching, cache-directory replacement containment, stale presentation-cache metadata containment, and single-project/delete-all Figma and presentation race tests. The complete Figma and presentation/provenance lifecycle lanes pass 171/171, the startup/PSD safety lane passes 21/21, and both the full serial source suite and fresh exact-base isolated Reprobox pass 329/329. The Keynote ambiguous-mojibake case passes 10/10 isolated serial repetitions. Final security, product/regression, and test-adequacy Autoreview lanes report no remaining finding.
- No package engine, watcher, parser result, provenance, diagnostics, renderer, quota, Figma scope, dependency, release, website, deploy, updater, build, signing, installed app, Keychain, real project, or external tester state changed.
- Completed the final read-only Autoreview, exact-base Reprobox, frozen-surface, syntax, whitespace, high-severity dependency, and static privacy checks with no remaining finding. Stopped before commit, push, PR, merge, build, installed-app validation, or Phase 5B pending the applicable approval. Olivia remains paused.

### 2026-07-15 Phase 5B Optional Diagnostics Minimization

- Began Phase 5B on `codex/security-diagnostics-minimization-phase5b`, based exactly on merged Phase 5A commit `2b75f38c7cc95e34117ea40c6481370569eedd6d` after the required Phase 5A installed-app gate completed.
- Replaced the broad optional diagnostic export with schema v2 aggregate package counts, fixed error categories, allowlisted graph metadata, fixed warnings, and fresh randomized report-local identifiers. Project identity, filenames, paths, timestamps, raw errors, payloads, credentials, URLs, Figma identifiers, and persistent graph IDs are omitted.
- Diagnostics remain off by default and retain `Crate Diagnostics/crate-provenance.json`; complete internal provenance, Package Details, package contents and counts, quota, Quick Package, watchers, parsers, Current Page Only, and Entire File behavior are unchanged.
- Initial independent Autoreview findings were fixed by randomizing report identifiers, updating legacy/schema-v2 operational readers, correcting privacy guidance and examples, and adding direct unknown-value, malformed-edge, fixed-category, and two-export coverage.
- Validation passes: focused lane 191/191, full serial suite 330/330, 20 legacy/schema-v2 reader cases, syntax, whitespace, and high-severity dependency gates. The only audit finding remains the pre-existing moderate `uuid` advisory; no dependency mutation occurred.
- Final exact-base closeout Reprobox: `/private/tmp/crate-reprobox-phase5b-closeout.Himav1`. The final proof reports its matching source/applied patch hash and isolated suite result without changing this frozen ledger entry.
...
- Final independent security and product/regression Autoreview lanes and the post-fix 330-test full source suite report no actionable P0-P3 finding. Phase 5B is ready for Bryant's commit, push, and PR approval. No build, installed-app mutation, release, deploy, updater, dependency, credential, or external tester action occurred. Olivia remains paused.

### 2026-07-16 Phase 6 Release Enforcement

- Phase 6A PR #139 merged into `v2.4.x` as `c9092fbdc715691f7ae2a06ca3ebc0dbef140721` after the source, exact-base Reprobox, and contained signed-app gates passed.
- Completed the Phase 6B source candidate on `codex/security-release-proof-phase6b` from that exact merge commit. It adds a read-only source CI gate, strict install-script policy, one approved Canvas prebuild path, privacy-safe notarization, and an exact-revision signed-app proof without changing normal app behavior.
- Final Autoreview found and fixed one malformed electron-builder context path that could emit a raw local TypeError before notarization. Invalid context, output paths, and product names now fail with a fixed path-free error, backed by direct regression tests.
- Final local validation passes 60/60 focused release/install tests and 394/394 complete serial source tests in both the working Reprobox and exact-base clone `/private/tmp/crate-phase6b-exact.YlV3zq/repo`. Changed-JavaScript syntax, workflow YAML, whitespace, credential scans, frozen manifest/lock hashes, and the high-severity audit gate also pass.
- The exact-base clone is byte-identical outside Git metadata and dependencies. Direct install-policy inspection correctly rejects the reused postinstall-populated Electron tree because it contains framework symlinks; a fresh Node 22 `npm ci --ignore-scripts` reconstruction was not authorized locally and is a mandatory PR-CI merge gate.
- No app runtime, renderer, package, watcher, parser, provenance, diagnostics, quota, Figma, dependency, or lockfile behavior changed. Bryant approved one commit, branch push, and PR #140 against `v2.4.x`; no merge, build, install, signing, notarization, release, deployment, updater, or tester mutation occurred. Olivia remains paused.
- The first GitHub Actions record for PR #140 failed before job creation because `runner.temp` is unavailable in job-level environment evaluation. The cache expression moved to only the two npm step environments, direct regression coverage requires that placement, and the corrected focused lane passes 60/60.
- The next run created the macOS Node 22 job, then pinned `actions/checkout` failed during credential cleanup because Crate's intentional `crate-web` and `mission-control` gitlinks have no `.gitmodules` mapping. The narrow correction keeps submodule fetching disabled, retains checkout credentials only through the pinned action, removes the GitHub authorization header in the immediately following step before any repository script runs, and writes local relative placeholder mappings only so checkout cleanup can traverse the legacy gitlinks without fetching them. A disposable exact-base probe plus release-policy, YAML, and whitespace checks pass locally.
- Final Autoreview required the mapping step to reject any pre-existing or symlinked `.gitmodules`, remain directly adjacent to checkout, and forbid conditional skipping or `continue-on-error`. The exact cleanup sequence removed a harmless synthetic authorization header and permitted cleanup traversal without initializing a nested repository; the focused release/install lane passes 60/60 after this guard.
- The following job passed checkout and credential scrubbing, then npm failed before install because its user and global configuration roles both resolved to `/dev/null`. The correction creates distinct owner-only empty config files under `RUNNER_TEMP` before setup-node and binds those files plus the isolated cache only to the install and audit steps. Exact workflow-policy coverage rejects job-wide config reuse, reordered or expanded preparation, conditional skipping, and incomplete npm-step binding; the focused lane passes 60/60 locally.
- GitHub also warns that the pinned checkout and setup-node v4 actions use a deprecated runtime. Their major-version migration is a separate pre-public-release CI-only task because checkout v7 changes credential persistence and requires a new failure-first cleanup proof; Phase 6B does not silently broaden into that migration.
- Next gate: rerun Autoreview, update PR #140, require corrected Node 22 CI and final merge readiness, then stop for separate merge approval. Repository branch enforcement, an independent code owner or release authority, and proof of a newly signed artifact remain separate pre-release blockers.
- After Phase 6B, address or explicitly disposition the pre-existing moderate `uuid` advisory before the final integrated six-phase audit closes.
- Continued the Phase 6B failure loop after the Codex memory-pressure pause. Two independent release-blocker Autoreviews found four real gaps: dependency audit ordering, exact mandatory-step coverage, invalid dual-`/dev/null` npm configuration in release/deploy playbooks, and mutable verifier-side `@electron/asar` / `@electron/fuses` execution dependencies. The workflow now audits immediately after frozen install before repository JavaScript, tests bind the exact mandatory step inventory and no-skip policy, and release/Cloudflare procedures create distinct owner-only npm configuration files.
- The release bootstrap now derives the exact 29-package asar/fuses execution closure from the committed lockfile, authenticates each installed package against registry integrity before either third-party module loads, rejects detached, malformed, substituted, preloaded, differently rooted, missing, or changed tool state, injects only authenticated handles, and rechecks tool bytes and committed verifier sources after execution. Failure-first tests prove pre-load and post-run drift fail closed with one privacy-safe error.
- Current post-fix validation passes 63/63 focused release/install tests and the then-current complete serial source suite with zero failures. This is local evidence only: final syntax/YAML/whitespace/privacy/audit checks, fresh independent Autoreview, amended PR #140 CI, and merge readiness remain pending. No merge, build, install, signing, notarization, release, deployment, dependency mutation, updater work, or Olivia rollout occurred.
- A later independent Autoreview pass found four additional release-proof gaps: the Canvas curl instruction omitted the pinned URL and destination, read-only gate wording did not separate mutation-free inspection from approved disposable test setup, CI did not exercise default verifier loaders against a fresh committed-lock install, and `releaseReady: true` could reach stdout before final byte-drift checks. All four are now corrected with direct failure-first policy or bootstrap coverage.
- The corrected focused unit lane passes 63/63. A disposable clean source copy completed `npm ci --ignore-scripts` and passed the real verifier-toolchain integration 1/1, authenticating the exact 29-package closure before loading asar/fuses and rechecking it afterward. The 398-test complete source suite reports 397 passed, zero failed, and one intentionally environment-gated integration skip; GitHub CI will execute that skipped lane against its own clean install.
- Final independent no-finding Autoreview, amended PR #140 CI, and merge readiness remain pending. No merge, build, app install, signing, notarization, release, deployment, dependency mutation, updater work, or Olivia rollout occurred.
- Both final independent Autoreview lanes now report no actionable findings and their subagents were closed. The amended PR #140 CI and merge-readiness review remain pending; no merge or release activity occurred.
- The amended clean GitHub job reached the complete source suite, where the existing real dependency-archive test failed because that step did not inherit the isolated npm cache populated by `npm ci`. The narrow correction adds only that cache binding and exact workflow-policy coverage.
- A disposable fresh-install source copy now passes the corrected 398-test complete suite with 397 passed, zero failed, and one environment-gated integration skip; the focused lane remains 63/63. Final Autoreview of this narrow correction, another amended CI run, and merge readiness remain pending.
- GitHub inline review then found one valid rollback defect: after a committed Canvas build failed post-commit validation, restoration could leave that new build inside the restored package. A failure-first regression reproduced the residue before the correction.
- The rollback now verifies and atomically detaches the exact committed build before restoring the original package, never follows replacement symlinks, preserves external sentinel targets, and supports a clean retry. A second failure-first test covers destination substitution both before restoration and after the restore rename; either race fails closed, and restored state is trusted only after the original package identity is reverified.
- Final local evidence reports 66 focused tests total with 65 passed, zero failed, and one CI-only integration skip; the complete suite reports 400 tests total with 399 passed, zero failed, and one intentionally environment-gated skip. Changed-source syntax, workflow YAML, whitespace, and protected-surface checks pass, and the final narrow independent Autoreview reports no actionable findings. Fresh amended PR #140 CI and final merge readiness remain required; no merge or release activity occurred.

### 2026-07-16 UUID Advisory Cleanup

- Phase 6B PR #140 merged into `v2.4.x` as `b44453c0854af64fa83cf5345054e34151d3b754` after fresh Node 22 CI, final merge readiness, and the addressed Canvas rollback review thread passed. No build, install, release, deploy, updater, or tester action occurred.
- Began the separately deferred moderate `uuid@9.0.1` advisory cleanup in isolated worktree branch `codex/security-uuid-cleanup` from that exact merge commit. The candidate replaces only the two existing `uuid.v4()` calls with Node `crypto.randomUUID()` and removes the direct dependency plus its root-only lock record.
- Failure-first source policy failed before implementation. Focused policy/lifecycle tests pass 17/17 and impacted main-process suites pass 190/190. Deterministic test harnesses now stub Node crypto while preserving all other crypto APIs.
- Exact-base Reprobox `/private/tmp/crate-uuid-reprobox.QD9xs4/repo` reconstructed 433 packages with lifecycle scripts disabled, passed the six-package install policy, reports zero vulnerabilities in full and production-only audits, and passes the Node `22.23.1` complete suite with 401 tests total, 400 passed, zero failed, and one intentional CI-only skip.
...
- Narrow ordinary-PR governance was applied and verified: `v2.4.x` is now the default branch; `Source security and regression suite` and PR flow are required with zero human approvals; admin enforcement and conversation resolution are enabled; force-pushes and branch deletion are disabled. This does not satisfy the stricter release gate: independently controlled code-owner approval and the layered branch/tag rulesets remain required before any version bump or distributable beta. Jenna does not need a GitHub account for installed-app QA and would need one only if selected as that independent release authority.
...
### 2026-09-19 Figma Scope and Initial Asset Baseline Correction

- Implemented the focused Beta 2.22 Figma fixes on `codex/figma-scope-baseline-correction-20260919` from the exact `v2.4.x` base `e390d3bce48d83abf8955fc131a74038a27fa06a`. Source commit `b9317ae` is in draft PR #264.
- Current Page Only preflight now requests the selected node without truncating the documented response ancestry; a missing selected node remains rejected. Figma-imported assets now stay Existing through incomplete initial scans and retries, successful empty scans establish the project baseline, and later unseen assets are Added while working. Existing origins and exclusions are preserved.
- Synthetic and focused validation passed: Figma scope 32/32, Figma link-per-project, Figma token privacy 9/9, renderer Figma scope 251/251, provenance helpers 7/7, and full provenance dual-write 578/578. Changed JavaScript syntax and whitespace checks passed; no private Figma link, live token, API response, owner project data, or installed app was used.
- The source security and regression workflow passed on the proof-state head before the subsequent review corrections. The corrective push will create a new PR head, so protected CI and two distinct independent Luna/high reviews must bind to that final head before handoff. PR remains draft; no build, installed-app regression, ready transition, merge, release, or deployment was performed.
- Both independent reviewers finished on PR head `22294d39e3d61dbe5ad164b9e81c4997ebf83cda`. They found a stale Figma scan could cross a link replacement/removal, pre-package recovery could clear its transfer block after incomplete ingestion, and package-time baseline transitions lacked direct assertions. Protected CI passed on that head, but its review evidence requested changes.
- Corrective source commit `7ee2e71` now binds polling and pre-package scans to a per-project Figma scope revision, verifies pending-baseline ownership before completion, retains the package block when ingestion is incomplete, and adds delayed replacement/removal plus package-baseline regressions. Local reruns passed `figma-link-per-project` 91/91, Figma scope/privacy/renderer 292/292, provenance helpers 7/7, and provenance dual-write 578/578. The source commit and proof-state update will be pushed together; all old-head CI/review evidence will be invalidated.
- The taskflow's next action is now to push the corrective head and rerun protected exact-head CI plus two fresh independent reviews. PR #264 remains draft; merge/ready, build, installed-app regression, release, and deployment remain outside this gate.
- The next two fresh reviews bound to `af8982825be2ae126b1213198fdb1e0e13cd57b0` found a remaining request-order race: a slow Figma link preflight could write an older replace after a later remove/replace. Source commit `7881740` advances a per-project link revision when each update starts and rejects superseded preflight completions; delayed replace/remove and replace/replace regressions now pass as part of `figma-link-per-project` 93/93. CI for the superseded `af89828` head was canceled; the next push requires new exact-head CI and two new read-only reviews.
- Two fresh Luna/high reviews on head `faa036d4a00bfb589a6bd12f7491804fac710dc0` found another scope-ordering window: a poll that began during replacement preflight retained the earlier revision through successful commit. Source correction `a1f1f39` advances the per-project revision again immediately before link mutation, fencing that delayed scan; its regression verifies the stale asset is rejected and the replacement link's first asset is classified Existing. The focused link suite passes 94/94, changed JavaScript syntax checks pass, and `git diff --check` is clean.
- CI run `35460465743` for the superseded `faa036d` head was canceled. The next correction push invalidates the passing CI and reviews on the already-pushed `4db79b1` head; the PR remains draft, with no build, full manual regression, merge, release, or deployment performed.
- The one Bryant-authorized additional correction fences per-project watcher scans before parser invocation and after token retrieval for the entire link preflight. Pre-package Figma recovery also fails closed while an update is pending; a successful watching-project update resumes its initial scan only after all overlapping preflights settle, and a failed preflight leaves the old link available for retry.
- Product correction `dc0304880a00c9204beadcb6d00a4887e1cd0088` was committed and pushed. Local Figma-link tests passed 96/96, combined provenance helper/dual-write tests passed 585/585, and syntax/whitespace checks passed. Protected CI run `35468009454` passed on that exact product head. Two distinct independent reviews found no runtime defects; one low finding concerned stale taskflow/current-workstream status.
- Bryant separately authorized this documentation-only closeout, normal commit/push, PR-body update, and required checks/reviews on the resulting head. The resulting documentation commit's CI/review outcomes and proof that all non-documentation files match `dc0304880a00c9204beadcb6d00a4887e1cd0088` are recorded externally in PR #264 and native task `01a0ba98-fede-7c91-85ab-caf5da7d880a` after commit. No additional commit is needed solely to record its own results; keep the PR draft and report product findings without expanding scope.
